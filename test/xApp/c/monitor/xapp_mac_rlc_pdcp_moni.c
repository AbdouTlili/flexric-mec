...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < nodes.len; i++) {
    ...
    mac_handle = report_sm_xapp_api(&nodes.n[i].id, n->ack_rf[0].id, i_0, sm_cb_mac);
    assert(mac_handle.success == true);
    sleep(1);

    rlc_handle = report_sm_xapp_api(&nodes.n[i].id, n->ack_rf[1].id, i_1, sm_cb_rlc);
    assert(rlc_handle.success == true);
    sleep(1);

    pdcp_handle = report_sm_xapp_api(&nodes.n[i].id, n->ack_rf[2].id, i_2, sm_cb_pdcp);
    assert(pdcp_handle.success == true);
    sleep(1);
  ...
  rm_report_sm_xapp_api(mac_handle.u.handle);

  rm_report_sm_xapp_api(rlc_handle.u.handle);

  rm_report_sm_xapp_api(pdcp_handle.u.handle);
