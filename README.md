...
* Install
  ...
  You can install the Service Models (SM) and the configuration file in your computer via:
  ```bash
  sudo make install
  ```
...
* 3 nodes test. See directory `build/examples/xApp/c/monitor`. This test check a simulated scenario using an E2 node's agent emulator that uses random data
  filled via `fill_kpm_ind_data()` function in the code. Example of run with 3 terminals for easying the console output:
    ```bash
...
You can also start wireshark and see how E2AP messages are flowing on the network interfaces.
...
We will use a patch provided in the flexric repository. Below the commands to that:
```bash
cd path/to/srsran
...
* Mailing list: if you need help or have some questions, you can subscribe to the mailing list `techs@mosaic-5g.io` that you can find at
  https://gitlab.eurecom.fr/mosaic5g/mosaic5g/-/wikis/mailing-lists. The emails are achived and available publicly. 
...
* [Flexric paper for ACM 2021](https://bit.ly/3uOXuCV)
